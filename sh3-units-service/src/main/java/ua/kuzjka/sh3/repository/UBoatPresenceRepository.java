...
public interface UBoatPresenceRepository extends JpaRepository<UBoatPresence, Integer>, PresenceRepository<UBoat> {
    ...
    @Override
    @Query("select p.uboat from UBoatPresence p where p.nation in :nations")
    List<UBoat> findByNations(@Param("nations") List<String> nations);

    @Override
    @Query("select p.uboat from UBoatPresence p where p.start <= :year and p.end >= :year and p.nation in :nations")
